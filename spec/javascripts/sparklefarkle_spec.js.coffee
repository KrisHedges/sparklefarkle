...
describe 'spark.winner(player, score)', ->
  before ->
    spark.winner(1, 10000)
...
describe 'spark.isYahtzee(facevalue, diceArray)', ->
  it "Should return true if the given value is found 6 times in the dice roll", ->
    dice = [2,2,2,2,2,2]
    spark.isYahtzee(2, dice).should.be.true
  it "Should return false if the given value is not found 6 times in the dice roll", ->
    dice = [2,2,2,2,2,1]
    spark.isYahtzee(2, dice).should.be.false
